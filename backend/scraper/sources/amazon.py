...
# Amazon blocks most scrapers; these selectors target the static HTML fallback
AMAZON_HEADERS = {
    **DEFAULT_HEADERS,
...
class AmazonScraper(BaseScraper):
    ...
    async def scrape(self, url: str, sku: str) -> ProductEnrichment:
        ...
        # Product name
        name_elem = soup.select_one('#productTitle') or soup.select_one('h1')
        product_name = name_elem.get_text(strip=True) if name_elem else None

        # Brand
        brand_elem = (
            soup.select_one('#bylineInfo')
            or soup.select_one('.po-brand .a-span9')
            or soup.select_one('[data-feature-name="brandLogo"]')
        )
        brand = brand_elem.get_text(strip=True).lstrip('Brand: ').lstrip('Visit the ').rstrip(' Store') if brand_elem else None

        # Certifications — often in the "Important information" or product detail sections
        certifications = []
        for badge in soup.select('[class*="cert"], [class*="badge"], .a-icon-prime'):
        ...
        # Allergens — Amazon often lists these in "Important information > Ingredients"
        allergen_contains = []
        ...
        # Ingredients — "Important information" section
        ingredients_raw = None
        ing_section = soup.select_one('#important-information') or soup.find(string=re.compile(r'\bingredients\b', re.I))
        if ing_section:
            if hasattr(ing_section, 'get_text'):
                ingredients_raw = ing_section.get_text(separator=' ', strip=True)
            else:
                parent = ing_section.find_parent(['div', 'section', 'p'])
                if parent:
