...
class VitaminShoppeScraper(BaseScraper):
    source = 'vitamin_shoppe'
    ...
    async def scrape(self, url: str, sku: str) -> ProductEnrichment:
        ...
        name_elem = (
            ...
            or soup.select_one('h1[data-testid="product-name"]')
            or soup.select_one('h1')
        )
        product_name = name_elem.get_text(strip=True) if name_elem else None

        brand_elem = soup.select_one('.brand-name, [data-testid="brand-name"]')
        ...
        return ProductEnrichment(
            ...
            brand=brand,
            certifications=extract_certifications(soup, ['.product-badges img', '.certification-icons img', '[class*="badge"]']),
