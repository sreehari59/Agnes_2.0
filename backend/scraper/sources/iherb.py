...
class IHerbScraper(BaseScraper):
    ...
    async def scrape(self, url: str, sku: str) -> ProductEnrichment:
        ...
        # Product name
        name_elem = (
            soup.select_one('h1[data-part="product-title"]')
        ...
        # Brand
        brand_elem = (
            soup.select_one('a[data-part="brand-link"]')
            or soup.select_one('.brand-link')
            or soup.select_one('[itemprop="brand"]')
        )
        brand = brand_elem.get_text(strip=True) if brand_elem else None

        # Certifications — badge images with alt text
        certifications = []
        for badge in soup.select('.certification-badge, .product-badge, [class*="cert"], [class*="badge"]'):
            text = badge.get('alt') or badge.get('title') or badge.get_text(strip=True)
            if text and len(text) < 80:
                certifications.append(text.strip())
        certifications = list(dict.fromkeys(certifications))  # deduplicate

        # Dietary claims — keyword scan
        ...
        # Allergens — "Contains: X, Y" pattern
        allergen_contains = []
        match = re.search(r'contains[:\s]+([^.;\n]+)', page_text, re.IGNORECASE)
        if match:
            raw = match.group(1)
            allergen_contains = [a.strip().rstrip(',') for a in raw.split(',') if a.strip()]

        # Free-from claims — "Free from X" or "X-free"
        allergen_free_from = []
