...
class SamsClubScraper(BaseScraper):
    ...
    async def scrape(self, url: str, sku: str) -> ProductEnrichment:
        async with httpx.AsyncClient(headers=DEFAULT_HEADERS, follow_redirects=True, timeout=20) as client:
        ...
        name_elem = (
            soup.select_one('h1.sc-product-title')
            or soup.select_one('[class*="productTitle"]')
